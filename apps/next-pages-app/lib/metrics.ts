...
/** Отправка метрики навигации (SPA или direct load). */
...
export function sendListRegenerationMetrics(params: ListRegenerationMetricsParams): void {
    const dataFetchMs = params.tDataReady - params.t0;
    const renderAfterDataMs = params.tDomReady - params.tDataReady;
    const totalMs = params.tDomReady - params.t0;
    ...
    postJson('/metrics/client', {
        ...
        marks_ms: {
            t0_user_action: 0,
            t_data_ready: dataFetchMs,
            t_dom_ready: totalMs,
        },
        derived_ms: {
            total: totalMs,
            data_fetch: dataFetchMs,
...
/** Отправка метрик первичной загрузки страницы (FCP, LCP, TTFB и ресурсы). */
