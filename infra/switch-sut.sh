...
if [ -z "$SUT" ]; then
  ...
  echo "Использование: ./switch-sut.sh <next-app|remix-app|remix3-app|vanilla-app|svelte-app|react-router-app|next-pages-app|astro-app|tanstack-app|solidstart-app|astro-vt-app|qwik-app>"
  exit 1
fi

echo "Останавливаем все SUT..."
docker rm -f next-app remix-app remix3-app vanilla-app svelte-app react-router-app next-pages-app astro-app tanstack-app solidstart-app astro-vt-app qwik-app 2>/dev/null || true
